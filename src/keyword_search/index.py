...
class TantivyIndex(BaseModel):
    ...
    def delete_if_exists(self):
        if self.full_path.exists():
            logger.info(f"Index {self.name} exists. Deleting...")
            import shutil
            shutil.rmtree(self.full_path)
            logger.info(f"Index {self.name} deleted successfully")
        else:
            logger.info(f"Index {self.name} does not exist")
        return True
    ...
    def create(self, mapping=None):
        ...
        logger.info(f"Index {self.name} created successfully")
        return True
    ...
    def upload_graph(self, graph: Graph, batch_size: int = 1000):
        ...
        logger.info(f"Total documents in {self.name}: {total_docs}")
...
if __name__ == "__main__":
    ...
    if not indices_dir.exists():
        logger.warning("No indices directory found")
        exit()
    ...
    for index_dir in index_dirs:
        ...
        try:
            ...
            if index._index:
                ...
                logger.warning(f"{index_name}: Failed to load")
    ...
    for index_name, doc_count, index in indices_info:
        ...
        try:
            ...
            if results['hits']['hits']:
                ...
                logger.info(f"Type: {source.get('type', 'N/A')}")
