...
doc_embeddings = torch.load(
    DATA_DIR
    / f"node_embeddings/{mag_graph.name}/text-embedding-ada-002/doc/candidate_emb_dict.pt"
)
query_embeddings = torch.load(
    DATA_DIR
    / f"node_embeddings/{mag_graph.name}/text-embedding-ada-002/query/query_emb_dict.pt"
)
...
for question_index, row in qas.iloc[:100].iterrows():
    ...
    candidate_df = pd.DataFrame(
        [
            (node.name, node.index, node.type, value)
            for node, value in candidate_rank.items()
        ],
