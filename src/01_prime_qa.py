...
qas = pd.read_csv(DATA_DIR / "02_qa_datasets/prime.csv")
...
os.makedirs(DATA_DIR / "connectedness/logs", exist_ok=True)
for question_index, row in qas.iloc[400:1000].iterrows():
    ...
    for entity in entities:
        nodes = prime_graph.search_nodes(entity, k=1)
        central_nodes.extend(nodes)

    neighborhoods = []
    for node in central_nodes:
        neighbors = prime_graph.get_neighbors(node)
    ...
    candidate_df = pd.DataFrame(
        [(node.name, node.index, value) for node, value in candidate_rank.items()],
    ...
    with open(DATA_DIR / f"connectedness/logs/{question_index}.json", "w") as f:
        json.dump(log, f, indent=4)
        candidate_df.to_csv(
            DATA_DIR / f"connectedness/logs/{question_index}_candidates.csv", index=False
        )